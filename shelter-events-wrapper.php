...
spl_autoload_register(
	function ( string $class_name ): void {
		...
		if ( str_starts_with( $class_name, 'Shelter_Events\\' ) ) {
				$relative = substr( $class_name, strlen( 'Shelter_Events\\' ) );
		...
		if ( str_starts_with( $class_name, 'Shelter_Events_' ) ) {
			$file = SHELTER_EVENTS_DIR . 'includes/class-' . strtolower( str_replace( '_', '-', $class_name ) ) . '.php';
			if ( file_exists( $file ) ) {
				require_once $file;
			}
		}
 ...
 *
 * Wires up the CPT, taxonomies, syncer, abilities, cron handler, admin
 * pages, blocks, REST routes, and front-end assets. Bails with an admin
 * notice when The Events Calendar is not active.
 */
function shelter_events_init(): void {
	...
	if ( function_exists( 'wp_register_ability_category' ) ) {
		...
		add_action( 'wp_abilities_api_init', array( \Shelter_Events\Abilities\Provider::class, 'register' ) );
	}

	// 5. WP-Cron handler — generate upcoming recurring event instances.
	add_action( 'shelter_events_generate_recurring', array( \Shelter_Events\Core\Event_Generator::class, 'run' ) );
	...
	// 8. REST routes.
	add_action( 'rest_api_init', array( 'Shelter_Events_REST', 'register_routes' ) );

	// 9. Override TEC cost display for variable-pricing events.
	add_filter(
		'tribe_get_cost',
		function ( string $cost, ?int $post_id ) {
			if ( ! $post_id ) {
				return $cost;
			}
			$variable = get_post_meta( $post_id, '_shelter_variable_pricing', true );
			return ( 'yes' === $variable ) ? __( 'Varies', 'shelter-events-wrapper' ) : $cost;
		},
		10,
		2
	);
