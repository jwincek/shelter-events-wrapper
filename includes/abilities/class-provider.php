...
final class Provider {
	...
	public static function handle_shelter_cancel_event( array $args ): array {
		...
		if ( ! $post || $post->post_type !== 'tribe_events' ) {
			return [ 'success' => false, 'error' => __( 'Event not found.', 'shelter-events-wrapper' ) ];
		}
	...
	public static function handle_shelter_replace_event( array $args ): array {
		...
		if ( get_post_meta( $event_id, '_shelter_replaced_by', true ) ) {
			return [ 'success' => false, 'error' => __( 'Event has already been replaced.', 'shelter-events-wrapper' ) ];
		}

		if ( get_post_meta( $event_id, '_shelter_cancelled', true ) ) {
			return [ 'success' => false, 'error' => __( 'Event is already cancelled.', 'shelter-events-wrapper' ) ];
		}
		...
		if ( ! $replacement instanceof \WP_Post ) {
			return [ 'success' => false, 'error' => __( 'Failed to create replacement event.', 'shelter-events-wrapper' ) ];
		}
